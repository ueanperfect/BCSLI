...
class mwindow(QWidget, UI.Ui_Form):
    ...
    def ExportPredictData_BP(self):
        ...
        if self.y_testPath=='/y_test.xlsx':
            ...
            self.y_pre=pd.DataFrame(self.model.predict(self.x_test))
            self.y_pre.to_excel(self.y_testPath)
    ...
    def show_train_data(self):
        ...
        self.tableWidget_zi_bian_liang.setHorizontalHeaderLabels(self.x_train.iloc[0].values)
        ...
        self.tableWidget_yin_bian_liang.setHorizontalHeaderLabels(self.y_train.iloc[0].values)
        for i in range(1, row):
